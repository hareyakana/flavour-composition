...
def load_sample_heatmap_data1(filename="heatmap110.txt"):
    ...
    for line in handle:
        line = line.strip()
        i, j, k, v = line.split(',')
        data[(round(float(i)), round(float(j)), round(float(k)))] = float(v)
    return data
...
d.set_title(r"source flavour composition, 2nd octant",fontsize=20)
...
d.heatmap(data4,style="hexagonal",cmap='OrRd',colorbar=None)                        
d.heatmap(data5,style="hexagonal",cmap='Oranges',colorbar=None)
...
#levels=[0.68,0.95]
#p.contour(w110,colors='white',levels=levels)