...
def coordinate(b,r,l):
    with open('ternary_coordinate400.txt','r')as f:
        reader = csv.DictReader(f)
        for row in reader:
            B=(float(400*float("%.4f" % float(row['x']))))
            R=(float(400*float("%.4f" % float(row['y']))))
            L=(float(400*float("%.4f" % float(row['z']))))
    return B,R,L
...
def F110(b,r,l):
    with open('110.txt','r')as f:
        reader = csv.DictReader(f)
        for row in reader:
            E110=(float(400*float("%.4f" % float(row['x']))))
            M110=(float(400*float("%.4f" % float(row['y']))))
            T110=(float(400*float("%.4f" % float(row['z']))))
    return E110,M110,T110
x,y,z=5,5,5
